...
function renderField(field, path) {
  ...
  else if (t === "imageUrl")          renderImageUrl(body, field, path);
  else if (t === "galleryUrl")        renderGalleryUrl(body, field, path);
  else if (t === "markdown")          renderTextarea(body, field, path);
  else if (t === "file")              renderFile(body, field, path);
  else if (t === "group") {
    // 嵌套 group
    ensurePath(data, path, {});
...
function makeEmptyItem(fields) {
  const o = {};
  for (const f of fields) {
    if (f.type === "list" || f.type === "gallery" || f.type === "galleryUrl") o[f.id] = [];
    else if (f.type === "group") o[f.id] = makeEmptyItem(f.fields || []);
    else if (f.type === "image" || f.type === "imageUrl" || f.type === "file") o[f.id] = null;
...
const POSTIMG_HINT = `
  <div class="url-hint">
    把图片上传到 <a href="https://postimg.cc/" target="_blank" rel="noopener">postimg.cc</a> →
    复制「<strong>Direct link</strong>」（以 <code>https://i.postimg.cc/</code> 开头）→
    粘贴到下方。
  </div>`;

function renderImageUrl(container, field, path) {
  const box = document.createElement("div");
  box.className = "url-img-box";
  box.innerHTML = POSTIMG_HINT;

  const inputRow = document.createElement("div");
  inputRow.className = "url-row";
  const input = document.createElement("input");
  input.type = "url";
  input.placeholder = "https://i.postimg.cc/xxxx/xxxx.jpg";
  const cur = getByPath(data, path);
  input.value = (cur && cur.url) || "";
  ...
  function drawPreview() {
    const v = getByPath(data, path);
    if (v && v.url) {
      preview.innerHTML = `<img src="${esc(v.url)}" alt=""><button class="rm" title="清空">×</button>`;
      ...
      preview.innerHTML = `<div class="url-preview__empty">（未粘贴链接）</div>`;
    }
  }
  input.addEventListener("input", e => {
    const v = e.target.value.trim();
    setByPath(data, path, v ? { url: v, alt: "" } : null);
    setDirty();
    drawPreview();
  });
  drawPreview();
...
function renderGalleryUrl(container, field, path) {
  ensurePath(data, path, []);
  const box = document.createElement("div");
  box.className = "url-gallery-box";
  box.innerHTML = POSTIMG_HINT;

  const grid = document.createElement("div");
  grid.className = "url-gallery-grid";
  ...
  function draw() {
    ...
    arr.forEach((it, idx) => {
      ...
      row.className = "url-gallery-row";
      row.draggable = true;
      row.dataset.idx = idx;
      row.innerHTML = `
        <div class="url-gallery-row__drag">⋮⋮</div>
        <div class="url-gallery-row__thumb">
          ${it.url ? `<img src="${esc(it.url)}" alt="" loading="lazy">` : `<div class="url-gallery-row__placeholder">—</div>`}
        </div>
        <div class="url-gallery-row__fields">
          <input class="url" type="url" placeholder="https://i.postimg.cc/..." value="${esc(it.url || "")}">
          ${field.caption === false ? "" : `<input class="cap" type="text" placeholder="caption（可留空）" value="${esc(it.caption || "")}">`}
        </div>
        <button class="rm" title="删除">×</button>
      `;
      const urlInput = row.querySelector(".url");
      urlInput.addEventListener("input", e => {
        it.url = e.target.value.trim();
        setDirty();
        const thumb = row.querySelector(".url-gallery-row__thumb");
        thumb.innerHTML = it.url
          ? `<img src="${esc(it.url)}" alt="" loading="lazy">`
          : `<div class="url-gallery-row__placeholder">—</div>`;
      });
      if (field.caption !== false) {
        row.querySelector(".cap").addEventListener("input", e => {
          it.caption = e.target.value;
          setDirty();
        });
      }
    ...
    wireDnd(grid, ".url-gallery-row", (from, to) => {
      const a = arr.splice(from, 1)[0];
      arr.splice(to, 0, a);
      setDirty();
      draw();
  ...
  const actions = document.createElement("div");
  actions.className = "url-gallery-actions";
  actions.innerHTML = `
    <button class="btn btn--sm" data-add>+ 新增一张</button>
    <button class="btn btn--sm" data-bulk>批量粘贴多行链接</button>
  `;
  actions.querySelector("[data-add]").addEventListener("click", () => {
    (getByPath(data, path) || []).push({ url: "", caption: "" });
    setDirty();
    draw();
  });
  actions.querySelector("[data-bulk]").addEventListener("click", () => {
    openBulkUrlDialog(getByPath(data, path), field.caption !== false, draw);
  });
...
function openBulkUrlDialog(arr, withCaption, onDone) {
  const dlg = document.createElement("div");
  dlg.className = "paste-dlg";
  dlg.innerHTML = `
    <div class="paste-dlg__box">
      <div class="paste-dlg__head">
        <strong>批量粘贴链接</strong>
        <span class="paste-dlg__hint">每行一个 URL${withCaption ? "，或「URL | caption」用竖线分隔" : ""}。现有列表将被追加。</span>
      </div>
      <textarea rows="14" placeholder="https://i.postimg.cc/xxx/a.jpg&#10;https://i.postimg.cc/yyy/b.jpg${withCaption ? " | 标题 b" : ""}"></textarea>
      <div class="paste-dlg__actions">
        <button class="btn" data-cancel>取消</button>
        <button class="btn btn--primary" data-apply>追加</button>
      </div>
    </div>`;
  const ta = dlg.querySelector("textarea");
  setTimeout(() => ta.focus(), 30);
  ...
  dlg.querySelector("[data-apply]").addEventListener("click", () => {
    ...
    for (const line of lines) {
      let url = line, caption = "";
      if (withCaption && line.includes("|")) {
        const i = line.indexOf("|");
        url = line.slice(0, i).trim();
        caption = line.slice(i + 1).trim();
      }
...
/* ---------- File (任意文件上传：视频/音频/PDF) ---------- */

function renderFile(container, field, path) {
  const box = document.createElement("div");
  ...
  function draw() {
    ...
    if (v && v.url) {
      ...
      const size = v.size ? ` · ${humanSize(v.size)}` : "";
      const typeLabel = (v.type || "").split("/")[0] || "file";
      card.innerHTML = `
        <div class="file-card__meta">
          <div class="file-card__name"><a href="${esc(v.url)}" target="_blank" rel="noopener">${esc(v.name || v.key || "file")}</a></div>
          <div class="file-card__sub">${esc(typeLabel)}${esc(size)}</div>
        </div>
        <button class="btn btn--sm btn--danger" data-rm>移除</button>
      `;
      card.querySelector("[data-rm]").addEventListener("click", async () => {
        if (!confirm("从 R2 删除此文件？")) return;
        if (v.key) await deleteR2(v.key).catch(() => {});
        setByPath(data, path, null);
        setDirty();
        draw();
      });
    ...
    actions.innerHTML = `<input type="file"${accept} /><button class="btn btn--sm" data-up>上传</button>`;
    const fileInput = actions.querySelector("input");
    actions.querySelector("[data-up]").addEventListener("click", async () => {
      if (!fileInput.files.length) return toast("先选文件", true);
      const f = fileInput.files[0];
      const btn = actions.querySelector("[data-up]");
      btn.disabled = true; btn.textContent = "上传中…";
      try {
        const [up] = await uploadFiles([f]);
        const old = getByPath(data, path);
        if (old && old.key) deleteR2(old.key).catch(() => {});
        setByPath(data, path, { key: up.key, url: up.url, name: up.name, size: up.size, type: f.type || "" });
        setDirty();
        draw();
        toast("已上传");
      } catch (e) {
